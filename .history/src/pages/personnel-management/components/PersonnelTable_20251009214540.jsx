...
const PersonnelTable = ({ personnel, onViewProfile, onEditPersonnel, onAssignPPE }) => {
  ...
  // Fetch empleados desde tu backend local
  useEffect(() => {
    const fetchPersonnel = async () => {
      try {
        const response = await fetch("/api/empleados"); // ✅ Gracias al proxy en vite.config.js
  ...
  const handleViewProfile = (employee) => {
    console.log("Ver perfil:", employee);
  ...
  const handleAssignPPE = (employee) => {
    console.log("Asignar EPP:", employee);
  };
