...
import FilterToolbar from './FilterToolbar'; // 🔹 Barra de filtros
...
const PersonnelTable = ({ onViewProfile, onEditPersonnel, onAssignPPE }) => {
  ...
  const filteredPersonnel = useMemo(() => {
    return (persons || []).filter((p) => {
      const matchSearch =
        !filters.search ||
        p.nombreCompleto?.toLowerCase().includes(filters.search.toLowerCase()) ||
        p.empleadoId?.toLowerCase().includes(filters.search.toLowerCase()) ||
        p.puesto?.toLowerCase().includes(filters.search.toLowerCase());

      const matchDepartment = !filters.department || p.departamento === filters.department;
      const matchStatus = !filters.status || p.estado === filters.status;
      const matchPosition = !filters.position || p.puesto === filters.position;
      const matchHireDateFrom =
        !filters.hireDateFrom || new Date(p.fechaIngreso) >= new Date(filters.hireDateFrom);
      const matchHireDateTo =
        !filters.hireDateTo || new Date(p.fechaIngreso) <= new Date(filters.hireDateTo);
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterToolbar
        ...
        onExportData={() => console.log('Exportar empleados')}
      />
