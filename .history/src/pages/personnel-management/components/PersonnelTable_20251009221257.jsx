...
const PersonnelTable = ({ onViewProfile, onEditPersonnel, onAssignPPE }) => {
  ...
  // 🔹 Cargar datos del endpoint /api/empleados
  useEffect(() => {
    const fetchPersonnel = async () => {
      try {
        ...
        const res = await fetch('/api/empleados'); // ✅ usa proxy de Vite

        if (!res.ok) throw new Error(`Error al obtener los empleados: ${res.status}`);

        const result = await res.json();
        console.log("📥 Respuesta backend:", result);

        // ✅ Verifica estructura del backend (success y data)
        if (result.success && Array.isArray(result.data)) {
          ...
          const mapped = result.data.map((emp) => ({
            ...
            email: emp.email,
            phone: emp.telefono,
            avatar: emp.avatar || '/default-avatar.png', // fallback
            lastUpdate: new Date(emp.updatedAt).toLocaleDateString(),
            medicalCompliance: emp.estado === 'Activo' ? 'Completo' : 'Pendiente',
            ppeCompliance: emp.estado === 'Activo' ? 'Completo' : 'Pendiente',
          }));

          setPersonnel(mapped);
        } else {
          console.warn("⚠️ Estructura inesperada en backend:", result);
          setPersonnel([]);
        }
      } catch (err) {
  ...
  // 🔹 Estado visual de los empleados
          ...
          <tbody className="bg-card divide-y divide-border">
            {sortedPersonnel.map((employee) => (
              <tr key={employee.id} className="hover:bg-muted/50 transition-smooth">
                ...
                <td className="px-6 py-4 whitespace-nowrap">{getComplianceBadge(employee.medicalCompliance)}</td>
                <td className="px-6 py-4 whitespace-nowrap">{getComplianceBadge(employee.ppeCompliance)}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">{employee.lastUpdate}</td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center space-x-2">
                    <Button variant="ghost" size="sm" onClick={() => onViewProfile(employee)} iconName="Eye" iconSize={16}>Ver</Button>
                    <Button variant="ghost" size="sm" onClick={() => onEditPersonnel(employee)} iconName="Edit" iconSize={16}>Editar</Button>
                    <Button variant="ghost" size="sm" onClick={() => onAssignPPE(employee)} iconName="Shield" iconSize={16}>EPP</Button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        {sortedPersonnel.map((employee) => (
          <div key={employee.id} className="bg-card border border-border rounded-lg p-4">
            ...
            <div className="space-y-2 mb-4">
              <div className="flex justify-between text-sm"><span className="text-muted-foreground">Departamento:</span><span>{employee.department}</span></div>
              <div className="flex justify-between text-sm"><span className="text-muted-foreground">Puesto:</span><span>{employee.position}</span></div>
              <div className="flex justify-between text-sm"><span className="text-muted-foreground">Estudios Médicos:</span>{getComplianceBadge(employee.medicalCompliance)}</div>
              <div className="flex justify-between text-sm"><span className="text-muted-foreground">EPP:</span>{getComplianceBadge(employee.ppeCompliance)}</div>
            </div>

            <div className="flex space-x-2">
              <Button variant="outline" size="sm" onClick={() => onViewProfile(employee)} iconName="Eye" iconSize={16} className="flex-1">Ver Perfil</Button>
              <Button variant="ghost" size="sm" onClick={() => onEditPersonnel(employee)} iconName="Edit" iconSize={16}>Editar</Button>
              <Button variant="ghost" size="sm" onClick={() => onAssignPPE(employee)} iconName="Shield" iconSize={16}>EPP</Button>
            </div>
          </div>
