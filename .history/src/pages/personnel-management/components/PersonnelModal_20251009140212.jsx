...
const PersonnelModal = ({ isOpen, onClose, employee, mode, onSave }) => {
  ...
  try {
    // 🔹 Construcción del payload tal como lo requiere tu backend
    const payload = {
      nombreCompleto: formData.nombreCompleto?.trim(),
      empleadoId: String(formData.empleadoId || "").trim(),
      email: formData.email?.trim().toLowerCase(),
      telefono: formData.telefono?.replace(/\D/g, ""), // Solo dígitos
    ...
    // 🔹 Validaciones básicas antes de enviar (opcional pero útil)
    ...
    // 🔹 Enviar al backend (Azure Function)
