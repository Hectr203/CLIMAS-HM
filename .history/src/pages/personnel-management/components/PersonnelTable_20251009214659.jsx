...
const PersonnelTable = ({ onViewProfile, onEditPersonnel, onAssignPPE }) => {
  ...
  if (loading) {
    return <div className="p-6 text-center text-muted-foreground">Cargando empleados...</div>;
  }

  if (sortedPersonnel.length === 0) {
    return <div className="p-6 text-center text-muted-foreground">No hay empleados registrados.</div>;
  }
          ...
          <tbody className="bg-card divide-y divide-border">
            {sortedPersonnel.map((employee) => (
              <tr key={employee?.id} className="hover:bg-muted/50 transition-smooth">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-foreground">{employee?.department}</td>
        ...
        {sortedPersonnel.map((employee) => (
          <div key={employee?.id} className="bg-card border border-border rounded-lg p-4">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center space-x-3">
                <div className="w-12 h-12 rounded-full overflow-hidden bg-muted">
                  <Image src={employee?.avatar || '/default-avatar.png'} alt={employee?.name} className="w-full h-full object-cover" />
                </div>
                <div>
            ...
            <div className="space-y-2 mb-4 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">Departamento:</span><span className="text-foreground">{employee?.department}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Puesto:</span><span className="text-foreground">{employee?.position}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Estudios Médicos:</span>{getComplianceBadge(employee?.medicalCompliance)}</div>
              <div className="flex justify-between"><span className="text-muted-foreground">EPP:</span>{getComplianceBadge(employee?.ppeCompliance)}</div>
            </div>
