...
const PersonnelTable = ({ onViewProfile, onEditPersonnel, onAssignPPE }) => {
  const { persons: rawPersons, loading, error } = usePerson(); // Usa el hook
  const [sortConfig, setSortConfig] = useState({ key: null, direction: 'asc' });

  // Mapeo de datos del backend a estructura local (useMemo para optimización)
  const personnel = useMemo(() => {
    return rawPersons.map((emp) => ({
      id: emp.id,
      ...
      lastUpdate: new Date(emp.updatedAt || emp.createdAt).toLocaleDateString(),
      medicalCompliance: emp.estado === 'Activo' ? 'Completo' : 'Pendiente',
      ppeCompliance: emp.estado === 'Activo' ? 'Completo' : 'Pendiente',
      // Agrega más campos si el backend los devuelve (e.g., medicalStudies, ppe, emergencyContact)
    }));
  }, [rawPersons]);

  // 🔹 Manejo de ordenamiento (igual que antes)
  ...
  // 🔹 Estado visual de los empleados (igual que antes)
  ...
  // 🔹 Mostrar mensajes de estado (usa loading y error del hook)
  if (loading) {
    return (
  ...
  // 🔹 Renderizado principal (igual que antes, pero con sortedPersonnel)
  return (
