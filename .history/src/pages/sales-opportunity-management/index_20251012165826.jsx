...
const SalesOpportunityManagement = () => {
  ...
  const mockOpportunities = [
    {
      ...
      priority: "high",
      stageDuration: 2,
      contactInfo: { phone: "+52 55 1234 5678", email: "contacto@corporacionabc.com", contactPerson: "Ing. Carlos Rodriguez" },
      projectDetails: { description: "Instalación de sistema HVAC para edificio corporativo", location: "Ciudad de México", estimatedBudget: 2500000, timeline: "3 meses" },
      documents: [],
      communications: [{ id: "comm-1", type: "whatsapp", date: "2024-03-28", content: "Cliente solicita cotización para sistema HVAC", urgency: "normal" }],
    ...
    },
    {
      ...
      priority: "medium",
      stageDuration: 5,
      contactInfo: { phone: "+52 55 9876 5432", email: "proyectos@industriasxyz.com", contactPerson: "Arq. Ana Martinez" },
      projectDetails: { description: "Mantenimiento y actualización de sistema existente", location: "Guadalajara, Jalisco", estimatedBudget: 850000, timeline: "6 semanas" },
      requirements: { complete: false, checklist: { technicalSpecs: true, locationAccess: false, budgetRange: true, timeline: true, permits: false } },
      communications: [{ id: "comm-2", type: "email", date: "2024-03-25", content: "Enviamos formulario de requerimientos técnicos", urgency: "normal" }],
    ...
    },
    {
      ...
      priority: "urgent",
      stageDuration: 8,
      contactInfo: { phone: "+52 55 4567 8901", email: "ventas@greenenergy.mx", contactPerson: "Ing. Fernando López" },
      projectDetails: { description: "Sistema HVAC para complejo residencial sustentable", location: "Monterrey, Nuevo León", estimatedBudget: 4200000, timeline: "4 meses" },
      requirements: { complete: true, checklist: { technicalSpecs: true, locationAccess: true, budgetRange: true, timeline: true, permits: true } },
      quotationData: { scope: "Instalación completa de sistema HVAC con controles inteligentes", assumptions: ["Acceso libre a todas las áreas durante horario laboral", "Cliente proporciona conexiones eléctricas principales"], timeline: "16 semanas", conditions: "50% anticipo, 50% contra entrega", materials: ["Unidades condensadoras", "Ductos", "Controles"], riskAssessment: "Medio - proyecto en construcción", extraCosts: [] },
      communications: [{ id: "comm-3", type: "whatsapp", date: "2024-03-20", content: "Cliente solicita ajustes en cronograma", urgency: "urgent" }],
    ...
    },
    {
      ...
      priority: "high",
      stageDuration: 12,
      contactInfo: { phone: "+52 55 2345 6789", email: "infraestructura@techsolutions.com", contactPerson: "Lic. Miguel Hernández" },
      projectDetails: { description: "Modernización de sistema de climatización para oficinas", location: "Puebla, Puebla", estimatedBudget: 1800000, timeline: "10 semanas" },
      quotationData: { scope: "Reemplazo de equipos obsoletos y optimización energética", assumptions: ["Trabajo nocturno y fines de semana", "Coordinación con otros contratistas"], timeline: "10 semanas", conditions: "30% anticipo, 40% avance 50%, 30% finalización", totalAmount: 1950000, validity: "30 días" },
      quotationStatus: { sent: true, sentDate: "2024-03-15", method: "email", attachments: ["Cotización_TechSolutions.pdf", "Cronograma.pdf"], clientFeedback: "Cliente solicitó ajustes menores" },
      communications: [{ id: "comm-4", type: "email", date: "2024-03-22", content: "Cliente acepta condiciones, solicita ajuste en cronograma", urgency: "normal" }],
    ...
    },
    {
      ...
      priority: "medium",
      stageDuration: 15,
      contactInfo: { phone: "+52 55 6789 0123", email: "desarrollo@urbandevelopment.mx", contactPerson: "Arq. Sofia Ramírez" },
      projectDetails: { description: "Sistema HVAC para centro comercial", location: "Tijuana, Baja California", estimatedBudget: 3500000, timeline: "5 meses" },
      quotationData: { totalAmount: 3750000, approved: true, approvalDate: "2024-03-26" },
      contractualInfo: { paymentConditions: "40% anticipo, 30% avance 50%, 30% finalización", billingData: { businessName: "Urban Development Group S.A. de C.V.", rfc: "UDG123456789", address: "Av. Revolución 1234, Tijuana, BC" }, deliverySchedule: [ { milestone: "Equipos principales", date: "2024-05-15" }, { milestone: "Instalación completa", date: "2024-07-30" } ] },
      workOrderGenerated: true,
      workOrderRef: "WO-2024-UDG-001",
      communications: [{ id: "comm-5", type: "email", date: "2024-03-26", content: "Contrato firmado. Iniciando proceso interno", urgency: "normal" }],
  ...
  const getDurationColor = (days) => days <= 3 ? 'text-green-600' : days <= 7 ? 'text-yellow-600' : 'text-red-600';
  ...
  return (
    <div className="min-h-screen bg-background flex">
        ...
        <div className="pt-16">
          <div className="container mx-auto px-4 py-8">
            ...
            <div className="relative flex transition-all duration-300">
              <div className={`flex-1 overflow-x-auto overflow-y-hidden pb-6 transition-all duration-300 ${showControls ? 'mr-[26rem]' : ''}`}>
                <div className="flex gap-6 min-w-max px-6">
                  {salesStages.map(stage => (
                    <div key={stage.id} className="flex flex-col w-[280px] bg-white rounded-2xl shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow">
                      ...
                      <div className="p-4 space-y-3 bg-gray-50 overflow-y-auto max-h-[calc(100vh-14rem)] scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
                        {getOpportunitiesByStage(stage.id)?.map(opportunity => (
                          <div key={opportunity.id} onClick={() => handleOpportunitySelect(opportunity)} className={`p-3 rounded-xl border-l-4 cursor-pointer hover:scale-[1.02] transform transition-all ${getPriorityColor(opportunity.priority)}`}>
                            ...
                            <div className="flex items-center justify-between text-xs text-gray-500 mb-1">
                              <div className="flex items-center space-x-2">
                                ...
                                <span className={`px-2 py-0.5 rounded ${opportunity.projectType === 'project' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>{opportunity.projectType === 'project' ? 'Proyecto' : 'Pieza'}</span>
                              </div>
                            </div>
              ...
              {showControls && (
                <div className="fixed top-[6rem] right-0 w-full max-w-[25rem] h-[calc(100vh-6rem)] bg-white rounded-l-2xl shadow-xl border-l z-20 overflow-y-auto">
                    ...
                    {selectedOpportunity && (
                      <div className="space-y-4">
                        ...
                        {selectedOpportunity.stage === 'initial-contact' && (
                          <ClientRegistrationPanel opportunity={selectedOpportunity} onRegister={data => handleClientRegistration(selectedOpportunity.id, data)} />
                        )}

                        <CommunicationPanel opportunity={selectedOpportunity} onAddCommunication={data => handleCommunicationAdd(selectedOpportunity.id, data)} />

                        {(selectedOpportunity.stage === 'quotation-development' || selectedOpportunity.quotationData) && (
                          <QuotationRequestPanel opportunity={selectedOpportunity} onUpdate={data => handleQuotationUpdate(selectedOpportunity.id, data)} />
                        )}

                        {selectedOpportunity.stage === 'closure' && selectedOpportunity.quotationData?.approved && (
                          <WorkOrderPanel opportunity={selectedOpportunity} onGenerateWorkOrder={data => handleWorkOrderGeneration(selectedOpportunity.id, data)} />
                        )}

                        {selectedOpportunity.stage !== 'initial-contact' && (
                          <ChangeManagementPanel opportunity={selectedOpportunity} onRequestChange={data => console.log('Change requested:', data)} />
                        )}
      ...
      <NewOpportunityModal isOpen={showNewOpportunityModal} onClose={() => setShowNewOpportunityModal(false)} onCreateOpportunity={handleCreateOpportunity} />
    </div>
  );
};
