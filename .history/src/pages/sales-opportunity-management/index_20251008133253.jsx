  ...
  <div className="flex-1">
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-6 mb-8">
      {salesStages?.map((stage) => (
        <div
          key={stage?.id}
          className="bg-card rounded-xl shadow-md border flex flex-col overflow-hidden"
        >
          {/* Header */}
          ...
          {/* Opportunities */}
          <div className="flex-1 p-4 space-y-3 min-h-[250px] max-h-[600px] overflow-y-auto">
            {getOpportunitiesByStage(stage?.id)?.map((opportunity) => (
              <div
                key={opportunity?.id}
                onClick={() => handleOpportunitySelect(opportunity)}
                className={`p-3 rounded-lg border-l-4 cursor-pointer bg-white hover:shadow-lg transition-all ${getPriorityColor(opportunity?.priority)}`}
              >
